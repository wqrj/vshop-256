...
public class ParameterValidatorUtil {
			...
			switch(validateRule) {
				...
				case "rand" : {
					if (!this.validateRand(val)) {
						this.errors.put(paramName, ActionMessageUtil.getMsg("validator.rand.msg")) ;
					}
					break ;
				}
			}
	...
	public boolean validateRand(String str) {
		if (this.validateString(str)) {
			return str.equalsIgnoreCase((String)ServletObjectUtil.getSession().getAttribute("rand")) ;
		}
		return false ;
	}
